...
Import `AutorenderUploader` and its stylesheet from the package. Give it the
**public key** (`AUTORENDER_PUBLIC_KEY`, exposed as `VITE_AUTORENDER_PUBLIC_KEY`):
it is visible to the browser by design and can only upload, so the widget is fine
on a public page. Never pass a private key (`AUTORENDER_API_KEY`) to the widget.

Use a server proxy instead when you need to validate the bytes, enforce a per-user
quota, or record uploads in your own database — see the Security Rules above.
