...
The browser widget is for internal/admin surfaces only because its unscoped key
is visible to the browser. For a server API key, use a Next.js route handler as a
server proxy and forward the multipart request to the documented upload API.
Never place `AUTORENDER_API_KEY` in a `NEXT_PUBLIC_` variable.
